...
legend_vertical_position = 0.7  # Controls the vertical position of the legend
data_label_vertical_position = 33  # Controls the vertical position of the data labels
arrow_vertical_position = 29  # Controls the vertical position of the double-headed arrow
...
for column in data.columns:
    ...
    shifted_average = average - 5
    shifted_std_dev_left = (average - std_dev) - 5
    shifted_std_dev_right = (average + std_dev) - 5
    ...
    plt.bar(edges[:-1] - 5, relative_frequency, width=bin_width, align='edge', 
    ...
    new_labels = [f'[{bins[i]}, {bins[i+1]})' for i in range(len(bins)-2)] + ['≥ 100']
    plt.xticks(np.arange(10, 110, 10), new_labels, fontsize=9, fontname='Calibri')
